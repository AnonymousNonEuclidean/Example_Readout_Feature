...
#                                trainable=True)
        ...
        # Transformed Leaky-ReLU Parameters
        #-----------------------------------#
#         self.select = self.add_weight(shape=[1],
#                         initializer = 'zeros',
#                         trainable=True)
    ...
    def call(self, inputs):
        ...
        output_activation = (1+tf.math.pow(self.flex_activ,2))*tf.nn.relu(output_spasely_connected_Layer) -(1-tf.math.pow(self.flex_activ,2))*tf.nn.relu(-output_spasely_connected_Layer) + tf.math.pow(self.flex_activ3,2)
        
        
        # Apply Depth Selection Parameter
        #--------------------------------#
#         part_identity = (1-tf.math.exp(self.select))*inputs
#         part_transformed = tf.math.exp(self.select)*output_activation
#         output_spasely_connected_Layer_selector = part_identity + part_transformed
...
class Rescaled_Leaky_ReLU(tf.keras.layers.Layer):
    ...
    def call(self,inputs):    
        # Activation
        output_activation = (1+tf.math.abs(self.flex_activ))*tf.nn.relu(inputs) -(tf.math.abs(self.flex_activ2))*tf.nn.relu(-inputs) + tf.math.abs(self.flex_activ3)
